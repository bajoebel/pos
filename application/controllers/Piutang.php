...
class Piutang extends CI_Controller {
    ...
    function __construct()
    {
        ...
        //$this->akses=$this->piutang_model->getAkses($level);
    }
	...
	public function index(){
        ...
        if($auth['priv_count']>0){
            ...
            /*$content=$this->load->view('admin/piutang/view_tabel', $data, true);
            $view=array(
                'content'   => $content
            );
            ...
            $content=array(
                ...
                'title' => 'Data Piutang',
                'isi' => $this->load->view('admin/piutang/view_tabel', $data,true)
            );
	...
	public function data(){
        ...
        if($auth['priv_count']>0){
            ...
            $limit = 20;
            $row_count=$this->piutang_model->countPiutang($q);
            $list=array(
                'status'    => true,
                'message'   => "OK",
                'start'     => $start,
                'row_count' => $row_count,
                'limit'     => $limit,
                'data'     => $this->piutang_model->getPiutanglimit($limit,$start,$q),
            );
        }else{
            $list=array(
                'status'    => false,
	...
	function edit($id=""){
        $auth=$this->_auth();
        if($auth['update']=='Y'){
            $row=$this->piutang_model->getPiutang_by_id($id);
            if(!empty($row)){
                $response=array(
                    'status'    => true,
                    'message'   => "OK",
	...
	function save(){
        ...
        if($auth['add']=='Y'){
            ...
            $data = array(
                ...
                'terima_piutangid' => $piutang_id,
                'terima_jumlah' => $this->input->post('jumlah_pembayaran'),
                'terima_userinput' => $auth['username'],
            );
             $insert = $this->piutang_model->insertPiutang($data);
                header('Content-Type: application/json');
    ...
    function histori($id=""){
        ...
        if($auth['priv_count']){
            $list=array(
                'data'     => $this->piutang_model->histori_bayar($id),
            );
            $res= $this->load->view('admin/piutang/histori_bayar', $list,true);
	...
	function delete($id){
        $auth=$this->_auth();
        if($auth['delete']=='Y'){
            $this->piutang_model->deletePiutang($id);
            header('Content-Type: application/json');
