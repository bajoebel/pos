<?php
Class Ass_jobpackage_model extends CI_Model
{
 ...
 function login($username, $password)
 {
   ...
   $this->db->where('ass_users.password', MD5($password));
   }
   if (strpos($username, '@') !== false) {
    $this->db->where('ass_users.email', $username);
   }else{
    $this->db->where('ass_users.wp_user', $username);
   }
   
   // $this -> db -> where('ass_users.verify_code', '0');
    $this -> db -> join('ass_group', 'ass_users.gid=ass_group.gid');
  $this->db->limit(1);
    $query = $this -> db -> get('ass_users');
 ...
 function resend($email){
  $this -> db -> where('ass_users.email', $email);
   // $this -> db -> where('ass_users.verify_code', '0');
    $this -> db -> join('ass_group', 'ass_users.gid=ass_group.gid');
  $this->db->limit(1);
    $query = $this -> db -> get('ass_users');
...
$verilink=site_url('login/verify/'.$veri_code);
 ...
 function recent_payments($limit){
   ...
   $this -> db -> join('ass_group', 'ass_payment.gid=ass_group.gid');
   $this -> db -> join('ass_users', 'ass_payment.uid=ass_users.uid');
  $this->db->limit($limit);
...
$p2=strtotime(date('Y',time()).'-'.$val.'-'.date('t',$p1));
 ...
 $query = $this->db->query("select * from ass_payment where paid_date >='$p1' and paid_date <='$p2'   ");
...
return $revenue;
 ...
 function login_wp($user)
 {
    ...
    $this -> db -> where('ass_users.wp_user', $user);
    $this -> db -> where('ass_users.verify_code', '0');
    $this -> db -> join('ass_group', 'ass_users.gid=ass_group.gid');
  $this->db->limit(1);
    $query = $this -> db -> get('ass_users');
 ...
 function insert_group(){
		...
		if($this->db->insert('ass_group',$userdata)){
  ...
  function update_group($gid){
		...
		if($this->db->update('ass_group',$userdata)){
 ...
 function user_list($limit){
	 if($this->input->post('search')){
		 ...
		 $this->db->or_where('ass_users.uid',$search);
		 $this->db->or_where('ass_users.email',$search);
		 $this->db->or_where('ass_users.first_name',$search);
		 $this->db->or_where('ass_users.last_name',$search);
		 $this->db->or_where('ass_users.contact_no',$search);

	 }
		$this->db->limit($this->config->item('number_of_rows'),$limit);
		$this->db->order_by('ass_users.uid','desc');
		 $this -> db -> join('ass_group', 'ass_users.gid=ass_group.gid');
		 $query=$this->db->get('ass_users');
		return $query->result_array();
 ...
 function verify_code($vcode){
	...
	$query=$this->db->get('ass_users');
		if($query->num_rows()=='1'){
			...
			$this->db->update('ass_users',$userdata);
			return true;
		}else{
 ...
 function insert_user(){
		...
		if($this->db->insert('ass_users',$userdata)){
		...
		$veri_code=rand('1111','9999');
		 if($this->config->item('verify_email')){
			$userdata['verify_code']=$veri_code;
				...
				$rresult=$this->db->update('ass_users',$userdata);
				if($this->session->userdata('logged_in_raw')){
				$this->session->unset_userdata('logged_in_raw');	
				}		
				}else{
				$rresult=$this->db->insert('ass_users',$userdata);
				}
		if($rresult){
			 if($this->config->item('verify_email')){
...
$new_password=rand('1111','9999');
 ...
 if($this->config->item('protocol')=="smtp"){
			...
			$this->db->where('email', $toemail);
 			$this->db->update('ass_users',$user_detail);
			return true;
			}
 ...
 function update_user($uid){
	 $logged_in=$this->session->userdata('logged_in');
		...
		if($this->input->post('user_status')){
		 ...
		 $this->db->where('uid',$uid);
		if($this->db->update('ass_users',$userdata)){
 ...
 function get_user($uid){
	...
	$this->db->where('ass_users.uid',$uid);
	   $this -> db -> join('ass_group', 'ass_users.gid=ass_group.gid');
$query=$this->db->get('ass_users');
	 return $query->row_array();
