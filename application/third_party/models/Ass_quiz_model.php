<?php
Class Ass_quiz_model extends CI_Model
{
 
  function quiz_list($limit){
	  
	  $logged_in=$this->session->userdata('info_user');
			if($logged_in['admin']=='Y'){
			$gid=$logged_in['gid'];
	 ...
	 if($this->input->post('search') && $logged_in['admin']=='Y'){
 ...
 function quiz_list_by_produk($produk_id,$limit){
 ...
 function product_list(){
	 ...
	 $query=$this->db->get('jp_job_produk');
	 return $query->result_array();
 ...
 function insert_quiz(){
	 ...
	 if($this->input->post('certificate_text')){
	  ...
	  $this->db->insert('ass_quiz',$userdata);
	 $quid=$this->db->insert_id();
	return $quid;
	 
 }
 
 function get_produk($quid){
 ...
 function update_quiz($quid){
	...
	'with_login'=>$this->input->post('with_login'),
	 ...
	 if($this->input->post('certificate_text')){
	  ...
	  $this->db->where('quid',$quid);
	  $this->db->update('ass_quiz',$userdata);
	  
	  $this->db->where('quid',$quid);
	  $query=$this->db->get('ass_quiz',$userdata);
	 $quiz=$query->row_array();
	 if($quiz['question_selection']=='1'){
		 
	  $this->db->where('quid',$quid);
	  $this->db->delete('ass_qcl');
                $correct_i=array();
        	 $incorrect_i=array();	 
	 foreach($_POST['cid'] as $ck => $val){
		 ...
		 $this->db->insert('ass_qcl',$userdata);
...
/*
	 if($cid!='0'){
		 $this->db->where('ass_qbank.cid',$cid);
	 }
	 if($lid!='0'){
		 $this->db->where('ass_qbank.lid',$lid);
	 }
*/
	  
	 $query=$this->db->query("select * from ass_qbank join ass_category on ass_category.cid=ass_qbank.cid join ass_level on ass_level.lid=ass_qbank.lid 
	 where ass_qbank.qid in ($qids) order by FIELD(ass_qbank.qid,$qids) 
	 ");
	 return $query->result_array();
 ...
 function get_options($qids){
	 ...
	 $query=$this->db->query("select * from ass_options where qid in ($qids) order by FIELD(ass_options.qid,$qids)");
	 return $query->result_array();
...
function insert_result($quid,$uid){
	...
	if($quiz['question_selection']=='0'){
		...
		$wqids=implode(',',$qids);
		$noq=array();
		$query=$this->db->query("select * from ass_qbank join ass_category on ass_category.cid=ass_qbank.cid where qid in ($wqids) ORDER BY FIELD(qid,$wqids)  ");	
		...
		foreach($qcl as $k => $val){
			...
			$i=0;
			$query=$this->db->query("select * from ass_qbank join ass_category on ass_category.cid=ass_qbank.cid where ass_qbank.cid='$cid' and lid='$lid' ORDER BY RAND() limit $noq ");	
			$questions=$query->result_array();
			foreach($questions as $qk => $question){
				$qids[]=$question['qid'];
				if(!in_array($question['category_name'],$categories)){
	...
	$this->db->insert('ass_result',$userdata);
 ...
 function open_result($quid,$uid){
	...
	$query=$this->db->query("select * from ass_result  where ass_result.result_status='$result_open'  and ass_result.uid='$uid'"); 
	if($query->num_rows() >= '1'){
		$result=$query->row_array();
		return $result['rid'];		
	}else{
 ...
 function quiz_result($rid){
	...
	$query=$this->db->query("select * from ass_result join ass_quiz on ass_result.quid=ass_quiz.quid where ass_result.rid='$rid' "); 
	return $query->row_array(); 
...
function saved_answers($rid){
	...
	$query=$this->db->query("select * from ass_answers  where ass_answers.rid='$rid' "); 
	return $query->result_array(); 
 ...
 function assign_score($rid,$qno,$score){
	...
	if($percentage_obtained >= $quiz['pass_percentage']){
	 ...
	 $this->db->update('ass_result',$userdata);
	 ...
	 $qp=$r_qids[$qno];
		...
		if($$qp_score=='1'){
		  ...
		  $query_qp="update ass_qbank set  $crin  where qid='$qp'  ";
	...
	function submit_result(){
		...
		foreach($qids_perf as $qp => $qpval){
			...
			if($qpval=='0'){
			  ...
			  $query_qp="update ass_qbank set no_time_served=(no_time_served +1)  $crin  where qid='$qp'  ";
		 	$this->db->query($query_qp);
		}
	 
		if($this->config->item('allow_result_email')){
			$this->load->library('email');
			$query = $this -> db -> query("select ass_result.*,ass_users.*,ass_quiz.* from ass_result, ass_users, ass_quiz where ass_users.uid=ass_result.uid and ass_quiz.quid=ass_result.quid and ass_result.rid='$rid'");
			$qrr=$query->row_array();
	...
	function insert_answer(){
		...
		foreach($_POST['answer'] as $ak => $answer){
			...
			if($_POST['question_type'][$ak] == '1' || $_POST['question_type'][$ak] == '2'){
				$qid=$qids[$ak];
				$query=$this->db->query(" select * from ass_options where qid='$qid' ");
				...
				foreach($answer as $sk => $ansval){
					...
					$this->db->insert('ass_answers',$userdata);
					$attempted=1;	
				}
				if($attempted==1){
					if($marks >= '0.99' ){
