<?php
Class Ass_qbank_model extends CI_Model
{
 
  function question_list($limit,$cid='0',$lid='0'){
	 if($this->input->post('search')){
		 $search=$this->input->post('search');
		 $this->db->or_where('ass_qbank.qid',$search);
		 $this->db->or_like('ass_qbank.question',$search);
		 $this->db->or_like('ass_qbank.description',$search);
	 ...
	 if($lid!='0'){
		 ...
		 $this->db->join('ass_category','ass_category.cid=ass_qbank.cid');
	 $this->db->join('ass_level','ass_level.lid=ass_qbank.lid');
	 $this->db->limit($this->config->item('number_of_rows'),$limit);
		$this->db->order_by('ass_qbank.qid','desc');
		$query=$this->db->get('ass_qbank');
 ...
 function remove_question($qid){
	 ...
	 $this->db->where('qid',$qid);
	 if($this->db->delete('ass_qbank')){
		  $this->db->where('qid',$qid);
			$this->db->delete('ass_options');
			
						
	$qr=$this->db->query("select * from ass_quiz where FIND_IN_SET($qid, qids) ");
			...
			$noq=count($nqids);
			$nqids=implode(',',$nqids);
			$this->db->query(" update ass_quiz set qids='$nqids', noq='$noq' where quid='$quid' ");
			}
 ...
 function insert_question_1(){
	 ...
	 $this->db->insert('ass_qbank',$userdata);
	...
	$userdata=array(
	 ...
	 $this->db->insert('ass_options',$userdata);	 
 ...
 function insert_question_5(){
  ...
  function update_question_1($qid){
	 ...
	 $this->db->where('qid',$qid);
	 $this->db->update('ass_qbank',$userdata);
	 $this->db->where('qid',$qid);
	$this->db->delete('ass_options');
	 foreach($this->input->post('option') as $key => $val){
 ...
 function category_list(){
	 ...
	 $query=$this->db->get('ass_category');
	 return $query->result_array();
 ...
 function update_category($cid){
		...
		if($this->db->update('ass_category',$userdata)){
 ...
 function remove_category($cid){
	 ...
	 $this->db->where('cid',$cid);
	 if($this->db->delete('ass_category')){
		 return true;
	 }else{
 ...
 function insert_category(){
		...
		if($this->db->insert('ass_category',$userdata)){
...
// level function start
 ...
 function update_level($lid){
		...
		if($this->db->update('ass_level',$userdata)){
 ...
 function insert_level(){
		...
		if($this->db->insert('ass_level',$userdata)){
...
$question_type=$this->lang->line('long_answer');	
	...
	if($this->db->insert('ass_qbank',$insert_data)){
		$qid=$this->db->insert_id();
		$optionkeycounter = 4;
		if($ques_type=="0" || $ques_type==""){
		for($i=1;$i<=10;$i++){
			if($singlequestion[$optionkeycounter] != ""){
				...
				$this->db->insert("ass_options",$insert_options);
				$optionkeycounter++;
				}
