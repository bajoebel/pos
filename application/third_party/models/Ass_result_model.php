<?php
Class Ass_result_model extends CI_Model
{
 ...
 function result_list($limit,$status='0'){
	...
	if($this->input->post('search')){
		...
		$this->db->join('ass_users','ass_users.uid=ass_result.uid');
 ...
 function generate_report($quid,$gid){
	...
	$date1=$this->input->post('date1');
	 $date2=$this->input->post('date2');
		...
		if($gid != '0'){
			$this->db->where('ass_users.gid',$gid);
		}
		if($date1 != ''){
		...
		if($date2 != ''){
	 	...
	 	$this->db->order_by('rid','desc');
		$this->db->join('ass_users','ass_users.uid=ass_result.uid');
		$this->db->join('ass_group','ass_group.gid=ass_users.gid');
		$this->db->join('ass_quiz','ass_quiz.quid=ass_result.quid');
		$query=$this->db->get('ass_result');
		return $query->result_array();
 }
