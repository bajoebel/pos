<div class="col-sm-12">
    <div class="row">
        <div class="col-sm-4">
            <a href="<?php echo base_url() ."return_barang" ?>" class="btn btn-success btn-sm">Ke Form Return</a>
        </div>
    ...
    <table class="table table-bordered" >
        <thead>
            <tr>
                <th style="width: 10px;">No</th><th>Pelanggan</th><th>Barang</th><th>Jumlah</th><th>Alasan</th><th>Harga Satuan</th><th>Harga</th><th>Tanggal</th>
    ...
    <script type="text/javascript">
        ...
        function view(start){
            ...
            $.ajax({
                url         : "<?php echo base_url() ."return_barang/data?q="; ?>" + search + "&start=" +start,
                type        : "GET",
                dataType    : "json",
                ...
                success     : function(data){
                    ...
                    for(a = 1; a < jmlData; a++){
                        // if(data[a]["jual_tipe"]=='Ecer') satuan=data[a]["barang_satuan_kecil"];
                        // else satuan=data[a]["barang_satuan_besar"];
                        if(data[a]["barang_diskon"]>0) diskon=(data[a]["barang_diskon"]/100)*data[a]["detail_harga_satuan"];
                        else diskon=0;
                        harga=data[a]["detail_harga_satuan"]-diskon;
                        //mencetak baris baru
                        buatTabel += "<tr>"
                            //membuat penomoran
                            + "<td>" + data[a]["start"] + "</td>"+ "<td>" + data[a]["pelanggan_nama"] + "</td>" +"</td>"+ "<td>" + data[a]["barang_nama"] + "</td>"+ "<td>" + data[a]["jml_return"] + " " + data[a]["detail_satuan"] + "</td>"+ "<td>" + data[a]["detail_alasan_return"] + "</td>"+"<td>" + "Rp. " +harga + "</td>"+ "<td>" + "Rp. " + data[a]["detail_return_harga"] + "</td>"+ "<td>" + data[a]["return_tanggal"] + "</td>"
        ...
        function remove(id)
        {
          if(confirm('Are you sure delete this data?'))
          {
            // ajax delete data from database
              $.ajax({
                url : "<?php echo base_url() .'return_barang/delete';?>/"+id,
                type: "POST",
                dataType: "JSON",
                success: function(data)
                {
